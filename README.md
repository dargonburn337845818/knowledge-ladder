...
- 每步自动选择信息增益最大的未问问题。
- 每次回答后展示「动态点拨」：由当前问题 + 你的回答对应到更深一层的算法结构理解，不是重复话术。
- 最终页按计算概率列出方向，进入详情后按“条件 → 动作 → 自问”三层递进解锁，并展示「深入点拨」。
...
- 8 档知识阶梯默认主界面：复习、复盘、统计三个独立入口。
- 算法卡 127 张：一句本质、题感信号、复杂度、C++ 模板；按「核心 / 常见 / 了解」分级，占位卡明确标为「思路框架」。
