...
- 软件不直接甩答案，最终给**四方向卡片 + 方向内三层递进点拨 + 卡点自查提示**，不展示候选算法权重，引导你自己收敛。
...
自动收敛 / 人类探测器打断
    ↓
四方向卡片平铺（编码压缩 / 传播松弛 / 剪枝决策 / 变换域映射）
    ↓
方向详情：三层递进点拨（条件 → 动作 → 自问）+ 卡点自查
    ↓
最小提示 + 最近卡点分布
```
...
- 最终页四张方向卡片平铺，点击进入详情；详情内三层点拨逐步解锁
- 方向页展示三层递进点拨与最近卡点分布（本地 localStorage 持久化）
...
- 四方向卡片平铺，点入详情后三层递进点拨；方向页提供卡点自查与最近卡点分布；不展示算法清单/权重
- 支持回退、重新开始、隐藏调试信息
...
> 最终页四张卡片同屏；点入详情后按“条件 → 动作 → 自问”三层递进解锁，不一次性剧透。
